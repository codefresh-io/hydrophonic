...
module.exports = function ({
    ...
    return {
        ...
        pullImage: ({ id, username, password })=> {
            let [image, tag = "latest"] = id.split(':');
            return client(Object.assign({
                    path: "images/create",
                    method: "POST",
                    qs: { fromImage: image, tag }
                }, username && password && { "headers": { "X-Registry-Auth": Buffer.from(JSON.stringify({ username, password })).toString('base64') }}))
